...
void StandardRasterizer::drawTriangle(Vertex &a, Vertex &b, Vertex &c, Shader *shader)
{
	...
	for (int i = 0; i < int(downTrianglePixelsLeft.size()); i++)
	{
		...
		for (int j = start; j <= end; j++)
		{
			Vector4 p(j, downTrianglePixelsLeft[i].y, 0, 0);
			u = cross(tmp_b, tmp_c, p) / area;
			v = cross(tmp_c, tmp_a, p) / area;
			w = 1 - u - v;
			Vertex fragment = Vertex::mix(a, b, c, u, v, w);
...
// 	for (int i = 0; i < int(upTrianglePixelsRight.size()); i++)
// 	{
// 		int start, end;
// 		if (i < int(upTrianglePixelsLeft.size()))
// 		{
// 			if (upTrianglePixelsLeft[i].x <= upTrianglePixelsRight[i].x)
// 			{
// 				start = upTrianglePixelsLeft[i].x;
...
// 		else
// 		{
// 			if (downTrianglePixelsLeft[i - int(upTrianglePixelsLeft.size())].x <= upTrianglePixelsRight[i].x)
// 			{
// 				start = downTrianglePixelsLeft[i - int(upTrianglePixelsLeft.size())].x;
// 				end = upTrianglePixelsRight[i].x;
// 			}
// 			else
// 			{
...
// 			w = 1 - u - v;
// 			Vertex fragment = Vertex::mix(a, b, c, u, v, w);
// 			if (depthTest(upTrianglePixelsRight[i].y, j, fragment) == false) continue;
// 			shader->fragment(fragment, this->target->pixelColor(j, upTrianglePixelsRight[i].y));
// 		}
// 	}
// }
